...
if __name__ == "__main__":
    ...
    for i in range(n_imgs):
        ...
        if i == 0:
            ...
            first_Points3D = volumes.get_cloud_ICP()

            print('1st', first_Points3D.shape[0], '2nd', second_Points3D.shape[1])
            if first_Points3D.shape[0] > second_Points3D.shape[1]:
                ind = random.sample(range(first_Points3D.shape[0]), second_Points3D.shape[1])
                pose, distances, _ = icp(second_Points3D.T, first_Points3D[ind, :])
            else:
                ind = random.sample(range(second_Points3D.shape[1]), first_Points3D.shape[0])
                pose, distances, _ = icp(second_Points3D.T[ind, :], first_Points3D)
        ...
        volumes.Fuse(color_image, depth_im, cam_intr, cam_pose, index=i)
